...
var table_informasi2 = $("#table_informasi2").DataTable({
 ...
 var table_biaya1 =  $("#table_biaya1").DataTable({
        responsive:true,
        stateSave: true,
...
var table_biaya2 = $("#table_biaya2").DataTable({
        responsive:true,
        stateSave: true,
...
var table_biaya3 =    $("#table_biaya3").DataTable({
        responsive:true,
        stateSave: true,
...
var table_batal_muat = $("#table_batal_muat").DataTable({
        responsive:true,
        stateSave: true,
...
var  table_alih_kapal =   $("#table_alih_kapal").DataTable({
        responsive:true,
        stateSave: true,
...
function fun_pilih_size_in(val) {
    var filter = [];
    filter = $("#pilih_size_in").val();
    console.log(filter);

    if (filter == null) {
        table_biaya1.columns(2).search("").draw();
    } else {
        table_biaya1
            .columns(2)
...
function fun_pilih_type_in(val) {
    var filter = [];
    filter = $("#pilih_type_in").val();
    console.log(filter);

    if (filter == null) {
        table_biaya1.columns(3).search("").draw();
    } else {
        table_biaya1
            .columns(3)
            .search(filter.join("|"), true, false, true)
...
function pilih_status_if1_fun(val) {
    var filter = [];
    filter = $("#pilih_status_if1").val();
    console.log(filter);

    if (filter == null) {
        table_informasi1.columns(1).search("").draw();
    } else {
        table_informasi1
            .columns(1)
            .search(filter)
